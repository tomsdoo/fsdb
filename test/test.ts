...
describe("FileDb", () => {
  ...
  it("value after initializing is empty object", async () => {
    assert.equal(
      await fileDb.get().then((obj) => JSON.stringify(obj)),
      JSON.stringify({})
    );
  });

  it("value of item that does not exist is undefined", async () => {
    assert.equal(await fileDb.get(uuid()), undefined);
  });
  ...
  it("save() update item", async () => {
    ...
    assert.equal(
      await fileDb.get(saved._id).then(({ testValue }) => testValue),
      "value after updating"
    );
  });

  it("getIds() returns a Promise that resolves string array", async () => {
    assert((await fileDb.getIds()).every((key) => typeof key === "string"));
  });
  ...
  it("remove()", async () => {
    ...
    assert.equal(key in (await fileDb.get()), false);
  });
});
